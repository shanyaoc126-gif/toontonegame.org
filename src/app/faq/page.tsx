    ...
    items: [
      {
        ...
        a: <>Yes — and the lab is built with you in mind. On your first visit (or any time via the CALIBRATE button) you can switch on <strong>color vision assist</strong>: every swatch then carries a shape code — six anchor hues map to six shapes, hues between two anchors carry both — shown on the target and on your print, so matching shapes means matching hue without betting on eyesight alone. The assist is a workaround, not a cure, and ToonTone remains a matching game, not a vision test: it can&apos;t diagnose, screen, or certify anything about your eyes. If you have concerns, a clinician with proper test plates is the address — not a browser game.</>,
        aText: 'Yes — and the lab is built with you in mind. On your first visit (or any time via the CALIBRATE button) you can switch on color vision assist: every swatch then carries a shape code — six anchor hues map to six shapes, hues between two anchors carry both — shown on the target and on your print, so matching shapes means matching hue without betting on eyesight alone. The assist is a workaround, not a cure, and ToonTone remains a matching game, not a vision test: it can\'t diagnose, screen, or certify anything about your eyes. If you have concerns, a clinician with proper test plates is the address — not a browser game.',
      },
      {
        q: 'What is the calibration screen on first visit?',
        a: <>A two-step pre-flight check in proofing-room spirit: step one is a monitor check — a dark plate with a barely different dark patch nested inside; if you can just barely separate the two, your display holds shadow detail and the lab reads true for you. Step two offers the color vision assist toggle. The whole ritual is skippable with one click (SKIP CALIBRATION), and the CALIBRATE button in the header re-opens it any time. It changes no score and stores only two flags on your own device.</>,
        aText: 'A two-step pre-flight check in proofing-room spirit: step one is a monitor check — a dark plate with a barely different dark patch nested inside; if you can just barely separate the two, your display holds shadow detail and the lab reads true for you. Step two offers the color vision assist toggle. The whole ritual is skippable with one click (SKIP CALIBRATION), and the CALIBRATE button in the header re-opens it any time. It changes no score and stores only two flags on your own device.',
      },
      {
